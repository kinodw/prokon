...
function download (fileId, auth, token) {
  ...
  drive.files.get({
    ...
    console.log('Downloading %s...', metadata);
...
fs.readFile('./client_secret.json', function processClientSecrets(err, content) {
  ...
  // Authorize a client with the loaded credentials, then call the
  // Drive API.
  authorize(JSON.parse(content), (auth, token) => {
    download("0B2RvKA_Sxg4BNG9rYm9ncWxlcVU", auth, token)
  });
});
