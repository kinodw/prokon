  ...
  constructor(option){
    ...
    this.client.on('message', message => {
      ...
      if(message.body.key in self.contexts){
        ...
        console.log("message: ",message);
        var from = message.to === "*" ? this.settings.id : message.to;
        var response = {
          "from":  from,
          "to": message.from,
          "body": {
      ...
      if(self.isConnected) console.log("Connected");
        ...
        self.client.hello({"from": self.settings.id }, (req, res) => {
          ...
          if(self.isConnected){
            console.log("HELLO: " + (self.isConnected ? "OK" : "NO"));
            /* HeartBeat: 接続確認 */
            this.heart = setInterval(this.heartBeat.bind(self), 30000);
            this.on('ack', (req, res)=>{this.response(res);})
  ...
  */
  send(command, option, callback){
    this.connect().then(()=>{
      ...
      command = command === undefined ? "test" : command;
      option.from = option.from === undefined ? this.settings.id : option.from;
      option.to = option.to === undefined ? undefined : option.to;
      option.body = option.body === undefined ? {} : option.body;
      option.body.command = command === undefined ? "ack" : command;
      option.body.key = option.body.key === undefined ? this.generateRandomID() : option.body.key;
      option.body.content = option.body.content === undefined ? {} : option.body.content;
      option.body.response = option.body.response === undefined ? true : option.body.response;

      /* 送信処理 */
      this.client.send(option, (req, res) => {
        console.log("send mes", req);
        this.contexts[req.body.key] = { "message": req, callback: callback };
      });
    })
  }

  sendTo(to, command='ack', option={}, callback=()=>{}){
    option.to = to;
    this.send(command, option, callback);
  }

  /* ブロードキャスト送信 */
  broadcast(command, option, callback){
    this.sendTo("*" ,command, option, callback)
  }

  sendToGroup(command, option, callback){
    if(option.body.content.group){
      this.send('list', {
        to: "GroupManager",
        body: {
          content: {
            type: "group",
            group: option.body.content.group
          }
        }
      }, (req, res) => {
        console.log("Sent Group "+res.body.content.group+" List: ");
        res.body.content.members.forEach((e,i,a)=>{
          console.log(e);
          if(i !== a.length) this.sendTo(e.id, comamnd, option);
          else this.sendTo(e.id, comamnd, option, callback);
        })
      })
    }
  }
